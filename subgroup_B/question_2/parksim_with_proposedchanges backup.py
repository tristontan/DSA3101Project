...
RANDOM_SEED = 13
NEW_VISITOR_BATCHES = 100  # Scaled down number of customers so the run time doesnt take too long
VISITORS_PER_BATCH = 275
INTERVAL_BATCHES = 15.0  # Generate new customers roughly every x clicks
MIN_PATIENCE = 45  # Min. customer patience - from survey
MAX_PATIENCE = 60  # Max. customer patience - from survey
REPEAT_SAME_RIDE_TIME = 4 # time taken to exit a ride and visit the same ride again
MAX_NUMBER_OF_RIDES = 12
LENGTH_OF_STAY = 7*60  # guests leave after 9 hours (closing hour)
# stimulator starts at 0 minute and visitors leave at 9*60 minute, mimics 1000-1900 (9 hours) opening hour of USS
...
def adjustDesirability(attracts, current_time, current_position, lunch_start=1*60, lunch_end=3*60, dinner_start=8*60, dinner_end=9*60, happy_start=5*60, happy_end=7*60, base_weight=1.0):
    ...
    for attract in attracts.values():
        ...
        if lunch_start <= current_time <= lunch_end:
            ...
            if attract['type'] == 'restaurant':
                attract['adjusted_desirability'] *= 2  # Increase desirability by a factor for dinner
            elif attract['type'] == 'ride':
                attract['adjusted_desirability'] *= 1/2
        elif happy_start <= current_time <= happy_end:
            if attract['type'] == 'restaurant':
                attract['adjusted_desirability'] *= 5  # Increase desirability by a factor for happy hour
            elif attract['type'] == 'ride':
                attract['adjusted_desirability'] *= 1/5
...
# the scaling of the USS map we are using has 4550 squared units / minute for walking
def calculateTravelToAttraction(person,attract):
    ...
    return distance / 9100.0 + REPEAT_SAME_RIDE_TIME
    ...
    while not done:
        ...
        for _ in range(10):
            ...
            if customers[name]['visit_counts'].get(attract_name, 0) < 3:
                ...
                if attraction['type'] != 'restaurant' or customers[name]['restaurant_visits'] < 2:
        ...
        if attraction['type'] == 'restaurant':
            happy_start = 5*60
            happy_end = 7*60
            if happy_start <= env.now <= happy_end:
                attraction['revenue'] += random.randint(attraction['pricerange_min'], attraction['pricerange_max']) * 0.8
                customers[name]['spending'] += random.randint(attraction['pricerange_min'], attraction['pricerange_max']) * 0.8
            else:
                attraction['revenue'] += random.randint(attraction['pricerange_min'], attraction['pricerange_max'])
                customers[name]['spending'] += random.randint(attraction['pricerange_min'], attraction['pricerange_max'])
        ...
        done_time = env.now > LENGTH_OF_STAY
...
# It provides insights into the average number of rides taken, wait times,
# and how many visitors left due to reaching their maximum allowed time in the park
def printVisitorInformation():
    ...
    print('Visitors spent a total of $', spending, 'and an average of $', spendAvg,' they visited restaurants a total of', restaurantvisits, 'times, with an average of', restvisitAvg, 'each')
...
env.process(logDesirabilityEveryInterval(env, attracts, LENGTH_OF_STAY))
