...
def adjustDesirability(attracts, current_time, current_position, lunch_start=LUNCH_START, lunch_end=LUNCH_END, base_weight=1.0):
    for attract in attracts.values():
        ...
        if lunch_start <= current_time <= lunch_end:
            if attract['type'] == 'restaurant':
                attract['adjusted_desirability'] *= 4  # Increase desirability by a factor for lunch
            elif attract['type'] == 'ride':
                attract['adjusted_desirability'] *= 1/4
